...
export function WorkOrderViewer({ orderId, open, onClose }: Props) {
  ...
  return (
    <Dialog open={open} onOpenChange={(v) => { if (!v) handleClose(); }}>
      <DialogContent className="max-w-4xl h-[90vh] flex flex-col gap-0 p-0 overflow-hidden relative">
        ...
        {/* Signature pad — absolute inside DialogContent, within scroll-lock scope */}
        {showPad && (
          <div className="absolute inset-0 bg-black/60 flex items-center justify-center p-6" style={{ zIndex: 10 }}>
            <SignaturePad
