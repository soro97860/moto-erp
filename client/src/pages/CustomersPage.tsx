...
import {
  ...
  Clock, Wrench, Package, Plus, Loader2, AlertCircle,
} from 'lucide-react';
import {
  useCustomers, useCustomerHistory, useCreateCustomer,
    ...
    <div className={cn('bg-white rounded-xl border overflow-hidden transition-shadow', expanded && 'shadow-md')}>
      ...
      {expanded && (
        <div className="border-t px-4 pb-4">
          {customer.note && (
            <p className="text-xs text-gray-500 pt-3 pb-1">備註：{customer.note}</p>
          )}
...
export function CustomersPage() {
  ...
  const [form, setForm] = useState({
    name: '', phone: '', licensePlate: '', vehicleModel: '', vehicleColor: '', note: '',
  });
  ...
  async function handleCreate() {
    if (!form.name || !form.phone || !form.licensePlate) {
      toast('請填寫必填欄位', 'error');
      return;
    }
    try {
      ...
      toast('車主已新增', 'success');
      setAddOpen(false);
      setForm({ name: '', phone: '', licensePlate: '', vehicleModel: '', vehicleColor: '', note: '' });
    } catch (err: unknown) {
        ...
        <DialogContent className="max-w-md">
          ...
          <div className="grid grid-cols-2 gap-3 text-sm">
            <div className="space-y-1">
              <Label>姓名 *</Label>
              <Input value={form.name} onChange={f('name')} placeholder="王小明" />
            </div>
            <div className="space-y-1">
              <Label>電話 *</Label>
              <Input value={form.phone} onChange={f('phone')} placeholder="0912-345-678" />
            </div>
            <div className="space-y-1">
              <Label>車牌號碼 *</Label>
              <Input
            ...
            <div className="space-y-1">
              <Label>車型</Label>
              <Input value={form.vehicleModel} onChange={f('vehicleModel')} placeholder="YAMAHA BWS 125" />
            </div>
            <div className="space-y-1">
              <Label>車色</Label>
              <Input value={form.vehicleColor} onChange={f('vehicleColor')} placeholder="白色" />
            </div>
